...
let typerfile_to_elexps f str lctx rctx =
  let pres = (f str) in
  ...
  (* FIXME fold left or right ?? *)
  let rec single_list_elexp elexpss = List.fold_left List.append [] elexpss in
  let rctx = List.fold_left add_to_rctx rctx (single_list_elexp elxpss) in
  (elxpss, lctx, rctx)
...
let rec filenames_to_elexps files_names lctx rctx = match files_names with
  | str::strs
    -> let (elexps, lctx, rctx) = compile_file str lctx rctx in
    let (_elexps, _lctx, _rctx) = filenames_to_elexps strs lctx rctx in
    (elexps @ _elexps , _lctx, _rctx)
  | [] -> [], lctx, rctx
